...
#Estimated Run Time ~ 60 - 120 Minutes with 10mb/s downloadspeed.
...
then
  ...
  for X in $y; do
    sed -i "${X}s/= /= $fallow_argument $boz_argument /g" $HOME/WRF/Downloads/NCEPlibs/macros.make.linux.gnu
  done
else
  echo "" 
...
then
  z="58 63"
  for X in $z; do 
    sed -i "${X}s/(FOPT)/(FOPT) $fallow_argument $boz_argument  /g" $HOME/WRF/UPPV4.1/configure.upp
  done
else
  echo "" 
  echo "Loop not needed"
...
tar -xvzf RIP_47.tar.gz -C $HOME/WRF/RIP4
cd $HOME/WRF/RIP4/RIP_47
mv * ..
cd $HOME/WRF/RIP4
rm -rd RIP_47
source $Miniconda_Install_DIR/etc/profile.d/conda.sh
...
export RIP_ROOT=$HOME/WRF/RIP4
export NETCDF=$DIR/NETCDF
export NCARG_ROOT=$HOME/WRF/miniconda3/envs/ncl_stable


sed -i '349s|-L${NETCDF}/lib -lnetcdf $NETCDFF|-L${NETCDF}/lib $NETCDFF -lnetcdff -lnetcdf -lnetcdf -lnetcdff_C -lhdf5 |g' $HOME/WRF/RIP4/configure

sed -i '27s|NETCDFLIB	= -L${NETCDF}/lib -lnetcdf CONFIGURE_NETCDFF_LIB|NETCDFLIB	= -L</usr/lib/x86_64-linux-gnu/libm.a> -lm -L${NETCDF}/lib CONFIGURE_NETCDFF_LIB -lnetcdf -lhdf5 -lhdf5_hl -lgfortran -lgcc -lz |g' $HOME/WRF/RIP4/arch/preamble

sed -i '31s|-L${NCARG_ROOT}/lib -lncarg -lncarg_gks -lncarg_c -lX11 -lXext -lpng -lz CONFIGURE_NCARG_LIB| -L${NCARG_ROOT}/lib -lncarg -lncarg_gks -lncarg_c -lX11 -lXext -lpng -lz -lcairo -lfontconfig -lpixman-1 -lfreetype -lexpat -lpthread -lbz2 -lXrender -lgfortran -lgcc -L</usr/lib/x86_64-linux-gnu/> -lm -lhdf5 -lhdf5_hl |g' $HOME/WRF/RIP4/arch/preamble

sed -i '33s| -O|-fallow-argument-mismatch -O |g' $HOME/WRF/RIP4/arch/configure.defaults

sed -i '35s|=|= -L$HOME/WRF/LIBS/grib2/lib -lhdf5 -lhdf5_hl |g' $HOME/WRF/RIP4/arch/configure.defaults
...
tar -xvf gsl_gwd.tar.gz -C $HOME/WRF/GEOG/WPS_GEOG
...
tar -xvzf geog_older_than_2000.tar.gz -C $HOME/WRF/GEOG/WPS_GEOG
